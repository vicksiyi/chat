...
Page({
  ...
  onLoad: function (options) {
    let _this = this;
    let dataUser = JSON.parse(options.user);
    this.setData({
      dataUser: dataUser
    });
    wx.getSystemInfo({
      ...
      success: function (res) {
        wx.getStorage({
          ...
          success: function (res2) {
            ...
            _this.connectStart(res.data, _name, _avatarUrl, dataUser.openId);
          },
          fail: function () {
            wx.redirectTo({
              url: '../auth/index',
  ...
  //与socket建立连接
  connectStart: function (_openid, _name, _avatarUrl, to) {
    var _this = this
    ...
    wx.onSocketError((err) => {
      ...
      errorConnectTimeOut = setTimeout(() => {
        _this.connectStart(_openid, _name, _avatarUrl, this.data.dataUser.openId)
      }, 2000)
    })
    // deal
    _this.deal()
    ...
    _this.sendSocketMessage({
      ...
      fail: function (res) {
        ...
        if (chatHeartBeatFailCount > 2) {
          ...
          _this.connectStart(_this.data._openid, _this.data._name, _this.data._avatarUrl, this.data.dataUser.openId);
        }
        if (heart) {
          chatHeartBeatTimeOut = setTimeout(() => {
            _this.heartBeat();
  ...
  deal: function () {
    ...
    chatWs.onMessage(onMessage => {
      ...
      if (data.type == '1v1Text' || data.type == '1v1Image' || data.type == '1v1Audio') {
